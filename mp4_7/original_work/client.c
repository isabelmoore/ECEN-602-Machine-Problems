...
int main(int argc, char *argv[]) {
    ...
    if (inet_pton(AF_INET, proxy_address, &proxy_addr.sin_addr) <= 0) {
        // check for invalid proxy address
    ...
    if (connect(client_socket, (struct sockaddr *)&proxy_addr, sizeof(proxy_addr)) < 0) {
        // check if connection to proxy server failed
    ...
    if (send(client_socket, request, strlen(request), 0) < 0) {
        // check if sending request to proxy server failed
    ...
    if (bytes_received < 0) {
        // check for error receiving response
